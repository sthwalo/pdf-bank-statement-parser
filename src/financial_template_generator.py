...
class FinancialTemplateGenerator:
    ...
    def _preprocess_transactions(self, df):
        """Add account codes and calculate balances."""
        # Add account codes
        ...
        # For transactions with Amount column, split into Debit/Credit
    ...
    def _generate_balance_sheet(self, df):
        """Generate balance sheet with asset, liability and equity sections."""
        
        balance_sheet = {
            'Assets': {
                'Current Assets': {
                    'Cash and Cash Equivalents': df[df['Account'].isin(['Bank', 'Cash'])]['Amount'].sum(),
                    'Accounts Receivable': df[df['Account'] == 'Accounts Receivable']['Amount'].sum(),
                    'Business Equipment': df[df['Account'] == 'Business Equipment']['Amount'].sum()
                },
                'Fixed Assets': {
                    'Vehicle': df[df['Account'] == 'Vehicle']['Amount'].sum(),
                }
            },
            'Liabilities': {
                'Current Liabilities': {
                    'Accounts Payable': df[df['Account'] == 'Accounts Payable']['Amount'].sum(),
                    'Bond Payments': df[df['Account'] == 'Bond Payments']['Amount'].sum()
                }
            },
            'Equity': {
                'Owner Equity': df[df['Account'] == 'Owner Equity']['Amount'].sum(),
                'Drawings': df[df['Account'] == 'Drawings']['Amount'].sum(),
                'Retained Earnings': self._calculate_retained_earnings(df)
            }
        }

        return balance_sheet

    def _generate_cash_flow_statement(self, df):
        """Generate cash flow statement with operating, investing and financing activities."""
        
        cash_flows = {
            'Operating Activities': {
                'Net Income': self._calculate_net_income(df),
                'Adjustments': {
                    'Depreciation': 0,  # Add if applicable
                    'Changes in Working Capital': {
                        'Accounts Receivable': self._calculate_working_capital_change(df, 'Accounts Receivable'),
                        'Accounts Payable': self._calculate_working_capital_change(df, 'Accounts Payable')
                    }
                }
            },
            'Investing Activities': {
                'Purchase of Equipment': df[(df['Account'] == 'Business Equipment') & 
                                         (df['Amount'] < 0)]['Amount'].sum(),
                'Vehicle Purchases': df[(df['Account'] == 'Vehicle') & 
                                      (df['Amount'] < 0)]['Amount'].sum()
            },
            'Financing Activities': {
                'Owner Drawings': df[df['Account'] == 'Drawings']['Amount'].sum(),
                'Bond Payments': df[df['Account'] == 'Bond Payments']['Amount'].sum()
            }
        }
    ...
    def _calculate_working_capital_change(self, df, account):
        """Calculate change in working capital accounts."""
        opening_balance = df[(df['Account'] == account) & 
                            (df['Date'] == df['Date'].min())]['Amount'].sum()
        closing_balance = df[(df['Account'] == account) & 
                            (df['Date'] == df['Date'].max())]['Amount'].sum()
    ...
    def _write_financial_statements(self, writer, income_stmt_data, balance_sheet_data, cash_flow_data):
        """Write all financial statements to Excel."""
        
        # Write Income Statement
        income_sheet = writer.book.add_worksheet('Income Statement')
        self._write_income_statement_section(income_sheet, income_stmt_data)
        
        # Write Balance Sheet
        balance_sheet = writer.book.add_worksheet('Balance Sheet')
        self._write_balance_sheet_section(balance_sheet, balance_sheet_data)
        
        # Write Cash Flow Statement
        cash_flow_sheet = writer.book.add_worksheet('Cash Flow')
        self._write_cash_flow_section(cash_flow_sheet, cash_flow_data)
        
        # Apply formatting to all sheets
        for sheet in [income_sheet, balance_sheet, cash_flow_sheet]:
            self._apply_excel_formatting(sheet)
    ...
    def generate_financial_statements(self, input_path, output_path):
        ...
        try:
            ...
            income_stmt_data = self._generate_income_statement(df)
            balance_sheet_data = self._generate_balance_sheet(df)
            cash_flow_data = self._generate_cash_flow_statement(df)
            
            # Create Excel writer
            with pd.ExcelWriter(output_path, engine='xlsxwriter') as writer:
                self._write_financial_statements(
                    writer, 
                    income_stmt_data, 
                    balance_sheet_data, 
                    cash_flow_data
...
def main():
    input_path = Path('Annual_Cashbook_FY2024-2025.xlsx')
